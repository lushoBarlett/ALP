...
num :: Integer -> LamTerm
num 0 = Abs "f" $ Abs "x" $ LVar "x"
num n = Abs "f" $ Abs "x" $ App (LVar "f") (App (App (num (n-1)) (LVar "f")) (LVar "x"))
...
parseVarAbs =
  do
      ide <- identifier untyped
      return (LVar ide)
    <|> do
          x <- lexeme untyped (decimal untyped) --lexeme toma un parser y devuelve otro que consume todos los espacios (tabuladores y newlines), a la derecha, del token parseado
